...
const ExportExcelGrade = ({ grades, selectedSubject, regularColumns, semesterId, classId, className }) => {
    ...
    const handleExportTemplate = async () => {
        ...
        worksheet.mergeCells('A2:K2');
        worksheet.getCell('A2').value = `BẢNG NHẬP ĐIỂM MÔN ${selectedSubject?.subjectName?.toUpperCase()} - LỚP ${className}`;
        worksheet.getCell('A2').alignment = { horizontal: 'center' };
        worksheet.getCell('A2').font = { bold: true, size: 13 };
        ...
        if (selectedSubject?.typeOfGrade === 'Nhận xét') {
            ...
            // Thêm dữ liệu mẫu (chỉ thông tin học sinh)
            grades.forEach((student, index) => {
                worksheet.addRow({
                    stt: index + 1,
                    studentId: student.studentId,
            ...
            // Thêm cột điểm giữa kỳ và cuối kỳ
            mainHeaders.push(
                { header: 'ĐĐG GK', key: 'GK', width: 8 },
                { header: 'ĐĐG CK', key: 'CK', width: 8 }
            );
        ...
        saveAs(blob, `Maunhapdiem_${selectedSubject?.subjectName}_Lop${className}_HK${semesterId === '7' ? 'I' : 'II'}.xlsx`);
    };
