...
const getTeacherName = (teacher_id) => {
  const teacher = teacherData.find(
    (t) => t.teacher_id === parseInt(teacher_id),
  );
...
const getSubjectName = (subject_Id) => {
  const subject = subjectData.find(
    (s) => s.subject_Id === parseInt(subject_Id),
  );
...
const ScheduleTable = () => {
  ...
  const [originalScheduleData, setOriginalScheduleData] =
                ...
                Object.entries(days).map(([day, sessions]) => [
                  ...
                  Object.fromEntries(
                    Object.entries(sessions).map(([session, periods]) => [
                      session,
                      periods.filter(
                        (period) => period.teacher_id === selectedTeacher,
                      ),
                    ]),
                  ),
                ]),
                ...
                Object.entries(days).map(([day, sessions]) => [
                  ...
                  Object.fromEntries(
                    Object.entries(sessions).map(([session, periods]) => [
                      session,
                      periods.filter(
                        (period) => period.subject_Id === selectedSubject,
                      ),
                    ]),
                  ),
                ]),
        ...
        <div className="filter-row">
          ...
          <div className="filter-column">
            ...
            >
              <option value="">-- Lựa chọn --</option>
              {tempGrade &&
                Object.keys(scheduleData[tempGrade]).map((className) => (
          ...
          <thead>
            ...
            <tr>
              ...
              {filteredClasses.map(({ grade, className }) => (
                <th key={`${grade}-${className}`}>
                  Khối {grade} - {className}
                </th>
              ))}
            </tr>
          </thead>
            ...
            {days.map((day, dayIndex) => {
              ...
              displayedSessions.forEach((session) => {
                let maxPeriods = Math.max(
                  ...filteredClasses.map(
                    ({ grade, className }) =>
                      filteredScheduleData[grade][className]?.[day]?.[session]
                        ?.length || 0,
                  ),
                );
                ...
                <React.Fragment key={day}>
                  {displayedSessions.map((session, sessionIndex) => {
                    let maxPeriods = Math.max(
                      ...filteredClasses.map(
                        ({ grade, className }) =>
                          filteredScheduleData[grade][className]?.[day]?.[
                            session
                          ]?.length || 0,
                      ),
                    );

                    return Array.from({ length: maxPeriods }).map(
                      (_, periodIndex) => {
                        let shouldDisplay = true;
                        const period =
                          filteredScheduleData[selectedGrade]?.[
                            selectedClass
                          ]?.[day]?.[session]?.[periodIndex];

                        if (
                          selectedTeacher &&
                          period &&
                          parseInt(period.teacher_id) !==
                        ...
                        return (
                          <tr
                            key={`${day}-${session}-${periodIndex}`}
                            className={rowClass}
                          >
                            {sessionIndex === 0 && periodIndex === 0 && (
                              <td
                                className="sticky-col col-1"
                                rowSpan={totalDisplayedPeriods}
                              >
                                {dayMap[day] || day}
                              </td>
                            )}
